...
use Eidsonator\BootstrapReportsBundle\Classes\ReportTypes\AdoReportType;
...
class VariableHeader extends HeaderBase
{
	...
	public static function afterParse(&$report) {
		$classname = "Eidsonator\\BootstrapReportsBundle\\Classes\\ReportTypes\\{$report->options['Type']}ReportType";
		...
		foreach($report->options['Variables'] as $var=>$params) {
			...
			if(isset($params['type']) && $params['type'] === 'daterange' && $report->macros[$params['name']]['start'] && $report->macros[$params['name']]['end']) {
